...
export class MaprRequestsService {
  ...
  update(maprRequests: IMaprRequests): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(maprRequests);
    return this.http
      .put<IMaprRequests>(`${this.resourceUrl}/${getMaprRequestsIdentifier(maprRequests) as number}`, copy, { observe: 'response' })
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
  }
  ...
  partialUpdate(maprRequests: IMaprRequests): Observable<EntityResponseType> {
    const copy = this.convertDateFromClient(maprRequests);
    return this.http
      .patch<IMaprRequests>(`${this.resourceUrl}/${getMaprRequestsIdentifier(maprRequests) as number}`, copy, { observe: 'response' })
      .pipe(map((res: EntityResponseType) => this.convertDateFromServer(res)));
  }
